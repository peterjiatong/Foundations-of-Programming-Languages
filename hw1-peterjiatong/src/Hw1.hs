...
additivePersistence n = checkCount n 0
    where checkCount x count | x < 10    = count
                             | otherwise = checkCount (sumList (digitsOfInt x)) (count + 1)
...
listReverse :: [a] -> [a]
listReverse x = helperFunction x []
    where helperFunction [] reversed     = reversed
          helperFunction (x:xs) reversed = helperFunction xs (x : reversed)
