...
eval :: Env -> Expr -> Value
--------------------------------------------------------------------------------
eval noNeed (EInt x) = VInt x

eval noNeed (EBool b) = VBool b

eval noNeed (ENil) = VNil
...
eval env (ELet id e1 e2) = eval currEnv e2
                where
                  val = eval currEnv e1
                  currEnv = ([(id, (val))] ++ env)
...
eval env (EApp expr1 expr2) = case (eval env expr1) of 
                ...
                noNeed -> throw (Error ("type error: EApp")) 
--------------------------------------------------------------------------------
evalOp :: Binop -> Value -> Value -> Value
--------------------------------------------------------------------------------
evalOp op (VInt x) (VInt y) = case op of
  Plus -> VInt (x + y)
  Minus -> VInt (x - y)
  Mul -> VInt (x * y)
