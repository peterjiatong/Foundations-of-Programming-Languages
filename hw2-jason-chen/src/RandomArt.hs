...
data Expr
  ...
  | TripleAverage Expr Expr Expr
  | ScaledSine Expr Expr
  deriving (Show)
...
exprToString (TripleAverage e1 e2 e3) = "((" ++ exprToString e1 ++ "+" ++ exprToString e2 ++ "+" ++ exprToString e3 ++ ")/3)"
exprToString (ScaledSine e1 e2)   = "sin(pi*" ++ exprToString e1 ++ "*" ++ exprToString e2 ++ ")"
...
eval x y (TripleAverage e1 e2 e3) = (eval x y e1 + eval x y e2 + eval x y e3) / 3
eval x y (ScaledSine e1 e2) = sin (pi * eval x y e1 * eval x y e2)
...
build 0
  ...
  where
    r         = rand 10
build d
  | r < 2     = Sine (Cosine (build (d - 1)))
  | r < 4     = Times (Sine (build (d - 1))) (Cosine (build (d - 1)))
  | r < 6     = Thresh (build (d - 1)) (build (d - 1)) (build (d - 1)) (build (d - 1))
  | r < 8     = TripleAverage (build (d - 1)) (build (d - 1)) (build (d - 1))
  | otherwise = ScaledSine (build (d - 1)) (build (d - 1))
  where
    r = rand 10
